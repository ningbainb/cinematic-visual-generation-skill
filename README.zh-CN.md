...
```text
使用 $cinematic-visual-generation，
为一个雨夜末班车故事生成 5 个独立电影分镜。
每个镜头单独生成，不要拼图，不要裁切。
```

当请求多个分镜时，技能默认输出多张独立图片，不会用联系表、九宫格或事后裁切代替原生镜头。

## 连续性锁定

多镜头故事会先建立一段 `Continuity Lock`，锁定以下内容并在每个镜头中重复：
...
每个镜头只改变动作、景别、机位和有动机的光线，这样故事能连贯推进，同时避免所有画面变成同一张照片的变体。

## 风格选择优先级

当不同风格要求冲突时，按下面顺序处理：

`用户明确要求 > 故事与情绪 > 选定的镜头配方 > 默认电影感`

每个镜头只选择一个主配方。比如“克制独立电影”不会自动叠加霓虹 noir 或史诗轮廓光，除非用户明确要求混合风格。
